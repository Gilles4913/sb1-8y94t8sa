...
import LoginSafety from '@/components/auth/LoginSafety' // veille à bien avoir ce fichier
...
export default function LoginPage() {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      // focus sur l’email pour pouvoir changer de compte facilement
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      // au cas où : purge d’une éventuelle session résiduelle
      await supabase.auth.signOut().catch(() => {})

      const { error } = await supabase.auth.signInWithPassword({ email, password })
      if (error) throw error

      // sortir de toute impersonation antérieure
      localStorage.removeItem('activeTenantId')
      localStorage.removeItem('activeTenantName')

      // route en fonction du rôle
      const role = await getUserRole()
      if (role === 'super_admin') nav('/admin', { replace: true })
      else nav('/clubs', { replace: true })
    } catch (e: any) {
      // affiche le message + code quand disponible
